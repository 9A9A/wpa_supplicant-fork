...
static u16 hostapd_parse_location_lci_req_age(const u8 *buf, size_t len)
{
	...
	/* Range req IE + Location subject + max age subelement */
	if (len < 3 + 1 + 4)
		return 0;
					 ...
					 u8 civic, u16 lci_max_age)
{
	...
	dl_list_for_each(nr, &hapd->nr_db, struct hostapd_neighbor_entry,
		...
		if (ssid->ssid_len != nr->ssid.ssid_len ||
		    os_memcmp(ssid->ssid, nr->ssid.ssid, ssid->ssid_len))
			continue;
		...
		if (send_lci && nr->lci) {
			...
			/* overriding measurement token - the first byte of the
			 * measurement IE
			 */
			msmt_token = wpabuf_mhead_u8(buf) + wpabuf_len(buf);
					  ...
					  const u8 *buf, size_t len)
{
	...
	while ((ie = get_ie(pos, len, WLAN_EID_MEASURE_REQUEST))) {
		...
		case MEASURE_TYPE_LCI:
			...
			lci_max_age =
				hostapd_parse_location_lci_req_age(ie + 2,
								   ie[1]);
			break;
				      ...
				      const u8 *buf, size_t len)
{
	...
	wpa_printf(MSG_INFO, "Radio measurement frame, action %d",
	...
	case WLAN_RRM_NEIGHBOR_REPORT_REQUEST:
		...
		break;
	default:
		wpa_printf(MSG_DEBUG, "RRM action %d is not supported",
			   mgmt->u.action.u.rrm.action);
	}
}
