...
def check_nr_results(dev, bssids=None, lci=False, civic=False):
	...
	found = False
        for r in received:
            if "RRM-NEIGHBOR-REP-RECEIVED nr=" + bssid.translate(None, ':') in r:
...
def test_rrm_range_req(dev, apdev):
    """hostapd range request APIs"""
    ...
    # responder not in DB
    # Note: this check would pass since the station does not support range
    # request and not because the responder is not in the DB
    if "FAIL" not in hapd.request("REQ_RANGE " + dev[0].own_addr() + " 10 10 00:11:22:33:44:55"):
        raise Exception("REQ_RANGE succeeded unexpectedly (responder not in DB)")
